...
class ChangeMobilePresenter(context: Context, var listener: ChangeMobileListener? = null) : BasePresenter(context) {
    ...
    interface ChangeMobileListener {
        ...
        fun verifyMobileSuccess(t: JsonObject)
    }
    ...
    fun checkMobile() {
        ...
        val observable = ApiManager.getInstance()
                .api.verifyOriginMobile(verify.get()!!)
        val observer = object : Observer<JsonObject> {
            ...
            override fun onNext(t: JsonObject) {
                ProgressDialog.hideLoadingView(context)
                listener?.verifyMobileSuccess(t)
            }

            override fun onError(e: Throwable) {
                ProgressDialog.hideLoadingView(context)
                ErrorManager.managerError(context, e, "验证手机失败，请重试")
            }
        }
        ApiManager.getInstance()
    ...
    fun changeMobile() {
        if (!Phone.isPhone(phone.get())) {
            ToastManager.showLongToast(context, "请输入正确手机号")
            return
        }
        if (!isVerifyCorrect) {
            ToastManager.showLongToast(context, "请输入验证码")
        ...
        val observable = ApiManager.getInstance()
                .api.changeMobile(phone.get()!!, verify.get())
        val observer = object : Observer<JsonObject> {
            override fun onComplete() {
